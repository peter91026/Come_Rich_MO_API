...
func (r *resolver) Web(input *loginModel.Login) interface{} {

	acknowledge, user, err := r.Account.AcknowledgeAccount(&accountsModel.Field{
	...
	token.Role = roleName //提供前端角色 過濾導覽列(事後寫入)
	token.Name = user.Name
	token.EngName = user.EngName
...
func (r *resolver) Refresh(input *jweModel.Refresh) interface{} {
	...
	if len(j.Token) == 0 {
		return code.GetCodeMessage(code.JWTRejected, "Refresh_token is null")
	}
	...
	var account = j.Other["account"].(string)
	user, err := r.Account.GetByAccount(&accountsModel.Field{
		Account:   util.PointerString(account),
